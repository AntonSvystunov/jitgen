...
class LarkStatementExtractor(ABC):
    ...
    invariant. Subclasses own the complete grammar-specific recovery policy by
    implementing `_parse_or_recover`.

    To support a new language, subclass this, set up the appropriate `Lark`
    parser, and implement `_parse_or_recover` with the exact set of Lark
    exceptions that mean "incomplete input, buffer more".

    Args:
        parser: A `Lark` parser configured with `propagate_positions=True` —
            required so parsed nodes carry the `meta.start_pos`/`end_pos`
            this class reads to slice statement text back out of the source.
    """
    ...
    def extract(
        ...
        # is additionally safe when it cannot grow: see `_is_sealed`.
    ...
    def _is_sealed(self, source: SourceCode, node: Branch[Token]) -> bool:
        """Check whether `node` could still be extended by input not yet arrived.
        ...
        * its grammar rule cannot take further clauses or an indented body —
          `_extensible_rules` names the ones that can; and
        * a real newline follows it in `source`, so the terminating NEWLINE came
          from the model rather than from the `"\\n"` that `_parse_or_recover`
          appends. Without this, `x = 1` would be released while the stream is
          still one chunk away from `x = 1 + 2`.

        Args:
            source: The full unexecuted-suffix buffer the parse tree came from.
            node: The last child of the parse tree — the only one this rule
                needs to check, since every earlier child is already fixed.

        Returns:
            `True` if `node` is sealed and safe to release immediately.
        """
    ...
    def _extensible_rules() -> frozenset[str] | None:
        """Name the rules whose nodes may still grow when more input arrives.
        ...
        Returns:
            The extensible rule names, or `None` to disable sealing.
        """
        return None

    @staticmethod
    def _statement_text(buffer: str, node: Branch[Token]) -> SourceCode:
        """Slice the source text spanned by `node` out of `buffer`.

        Args:
            buffer: The full unexecuted-suffix buffer the parse tree came from.
            node: The parse tree node to extract text for.

        Returns:
            The text `node` spans in `buffer`, or `""` if it carries no
            position metadata.
        """
        meta = getattr(node, "meta", None)
        if meta is None:
