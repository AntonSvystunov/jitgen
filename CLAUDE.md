...
What exists today is the **core protocol layer, one concrete segmenter, a Lark-based Python statement extractor, and an in-process Python executor** — every piece needed for a working end-to-end Python pipeline is now present, though nothing yet wires them together into a `create_python_session()`-style factory; check before assuming one exists. In particular, nothing in `jitgen` yet constructs the `Lark` parser `PythonLarkExtractor` needs (see the Architecture section below and `tests/unit_tests/conftest.py`'s `python_lark_parser` fixture for the construction incantation). `tests/unit_tests/test_session.py` and `test_driver.py` test `Session`/`StreamDriver` against small hand-written `StatementExtractor`/`BaseExecutor` doubles defined in `tests/unit_tests/conftest.py` (`FakeExtractor`, `FakeExecutor`) rather than a real grammar or interpreter — reuse those fixtures rather than inventing new doubles when extending these tests. `tests/unit_tests/extractors/test_python.py` covers `PythonLarkExtractor` against the real grammar, and `tests/unit_tests/executors/test_python.py` covers `InProcPythonExecutor` against a real (in-process) Python interpreter, via their own local fixtures.
